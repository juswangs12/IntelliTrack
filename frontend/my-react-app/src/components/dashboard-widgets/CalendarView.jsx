...
export function CalendarView({ deadlines = {}, onDateSelect }) {
      ...
      <div>
        ...
        <div className="grid grid-cols-7 gap-0 border border-border rounded-lg overflow-hidden">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
            <div
              key={day}
              className="bg-[#800020] text-white p-2 text-center text-sm font-semibold border border-[#9B1B30]"
            >
              {day}
            </div>
          ))}
  ...
  return (
    <Card className="border-2 border-[#800020]/20">
      ...
      <CardContent className="pt-6">{renderCalendar()}</CardContent>
    </Card>
  );
}
